...
<body>
	<form action="<c:url value="/employees-category"/> " method="post">
													...
													<c:forEach items="${category.getList()}" var="c"
														...
														<tr>
															<td style="text-align: center;"><a
																href="<c:url value="/employees-product-edit?id=${c.getId()}"/>">${c.getId()}
															</a></td>
															<td style="text-align: center;"><a
																href="<c:url value="/employees-product-edit?id=${c.getId()}"/>">${c.getName()}
															</a></td>
															...
															<td><a
																href="<c:url value="/employees-category-edit?id=${c.getId()}"/>"><button
																		type="button" class="btn btn-outline-success btn-sm">
																		<i class="ti-pencil"></i>
																	</button></a></td>
															<c:if test="${qty[s.count] == 0}">
																<td><a href="<c:url value="/employees-category-delete?action=xoa&id=${c.getId()}"/>"><button type="button"
