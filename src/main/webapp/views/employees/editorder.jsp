...
<body>
	<form action=" <c:url value="/employees-order-edit"/>" method="post">
						...
						<div class="card">
							...
							<div class="card-body">
								...
								<div class="row">
									...
									<div class="col-lg-4 col-md-6 col-6">
										<c:choose>
                            				<c:when test="${transaction.getStatus() == 0}">
												<select class="form-control" aria-placeholder="${transaction.getStatus()}" name="status">
													<option value="0">Chờ xác nhận</option>
													<option value="1">Chờ lấy hàng</option>
												</select>
											</c:when>
											<c:when test="${transaction.getStatus() == 1}">
												<select class="form-control" aria-placeholder="${transaction.getStatus()}" name="status">
													<option value="1">Chờ lấy hàng</option>
													<option value="2">Đang đi giao</option>
												</select>
											</c:when>
											<c:when test="${transaction.getStatus() == 2}">
												<select class="form-control" aria-placeholder="${transaction.getStatus()}" name="status">
													<option value="2">Đang đi giao</option>
													<option value="3">Đã giao</option>
												</select>
											</c:when>
											<c:when test="${transaction.getStatus() == 3}">
												<select class="form-control" aria-placeholder="${transaction.getStatus()}" name="status">
													<option value="3">Đã giao</option>
												</select>
											</c:when>
								...
								<div class="row">
									<div class="col-12" style="text-align: center;">
										...
										<a href="<c:url value="/employees-order"/>"><button
